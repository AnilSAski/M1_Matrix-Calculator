...
c c_division(c a,c b)
{
    ...
    if (b.img==0 && b.real==0)
    {
        ...
        b.img=b.img*-1;
        res.real=(1/(pow(b.real,2)+pow(b.img,2)))*(c_multiplication(a,b).real);
        res.img=(1/(pow(b.real,2)+pow(b.img,2)))*(c_multiplication(a,b).img);
        return res;
    }
...
double * shortLine(int rcv_pow,int rcv_vol ,double pf,double res, double inductor )
{
    ...
    r=c_multiplication(z,irbar);
    vs.real=rcv_vol+r.real;
    vs.img=r.img;
    send_vol=c_magnitude(vs);
...
double * mediumPiLine(int rcv_pow,int rcv_vol ,double pf,double res, double inductor,double cap,int len )
{
    ...
    Ilinebar=c_addition(Irbar,Ic1bar);
    r=c_multiplication(z,Ilinebar);
    vs.real=rcv_vol+r.real;
    vs.img=r.img;
    send_vol=c_magnitude(vs);
    ang1=c_angle(vs);
    ...
    Isbar=c_addition(Ic2bar,Ilinebar);
    send_curr=c_magnitude(Isbar);
    ang2=c_angle(Isbar);
...
double * mediumTLine(int rcv_pow,int rcv_vol ,double pf,double res, double inductor,double cap,int len )
{
    double sinteta,rcv_curr,eff,reg,send_vol,send_pow,ang1,ang2,send_curr,pfs,*ptr=arr;
    c z,Irbar,vs,r,Ic1bar,Ilinebar,Ic2bar,Isbar;
    rcv_vol=rcv_vol/sqrt(3);
...
double * longLine(int rcv_pow,int rcv_vol ,double pf,double res, double inductor,double cap,int len )
{
    ...
    irbar.real=rcv_curr*pf;
    irbar.img=-rcv_curr*sinteta;
    A.real=cosh(sqrt((int)(c_multiplication(ybar,z).real*-1)));
    B.real=sqrt(c_division(z,ybar).real)*sinh(sqrt(-1*c_multiplication(ybar,z).real));
    C.real=sqrt(c_division(z,ybar).real)*sinh(sqrt(-1*c_multiplication(ybar,z).real));
    D.real=A.real;
    A.img=0;B.img=0;C.img=0;D.img=0;
    vs=c_addition(c_multiplication(A,vrbar),c_multiplication(B,irbar));
    Isbar=c_addition(c_multiplication(C,vrbar),c_multiplication(D,irbar));
    send_vol=c_magnitude(vs);
    send_curr=c_magnitude(Isbar);
    send_pow=rcv_pow+((3*pow(send_curr,2)*res*0.5)+(3*pow(rcv_curr,2)*res*0.5));
    eff=(rcv_pow/send_pow)*100;
